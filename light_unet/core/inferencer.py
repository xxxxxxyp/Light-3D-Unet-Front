...
class Inferencer:
    ...
    def extract_bboxes(self, prob_map, threshold=0.3, min_volume_cc=0.5, spacing=(4.0, 4.0, 4.0)):
        ...
        for component_id in range(1, num_components + 1):
            ...
            bbox = {
                ...
                "bbox_voxel": [int(z_min_exp), int(z_max_exp), int(y_min_exp), int(y_max_exp), int(x_min_exp), int(x_max_exp)],
                "bbox_mm": [float(z_min_mm), float(z_max_mm), float(y_min_mm), float(y_max_mm), float(x_min_mm), float(x_max_mm)],
    ...
    def infer_case(self, case_id, data_dir, threshold=0.3):
        ...
        if apply_body_mask:
            ...
            if body_mask_path.exists():
                body_mask = nib.load(body_mask_path).get_fdata().astype(bool)
            else:
                print(f"Warning: Body mask not found for {case_id}")
        ...
        try:
            ...
            print(f"Error during inference execution for {case_id}: {e}")
            return False
        ...
        nib.save(nib.Nifti1Image(prob_map, affine, header), prob_map_path)
        
        bboxes = self.extract_bboxes(prob_map, threshold=threshold, min_volume_cc=self.config["data"]["volume_threshold"]["inference_cc"], spacing=spacing)
    ...
    def infer_split(self, split_file, data_dir):
        ...
        for case_id in tqdm(case_ids, desc="Inference"):
            if self.infer_case(case_id, data_dir, threshold=threshold):
                successful += 1
            else:
                failed.append(case_id)
        
        print(f"\nInference complete: Successful: {successful}/{len(case_ids)}")